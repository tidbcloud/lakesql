...
impl APIClient {
    ...
    async fn build_client(&mut self, name: Option<String>) -> Result<()> {
        let ua = match name {
            Some(n) => n,
            None => format!("lake-client-rust/{}", VERSION.as_str()),
        };
    ...
    async fn check_presign(self: &Arc<Self>) -> Result<()> {
        let mode = match self.get_presign_mode() {
            PresignMode::Auto => {
                if self.host.ends_with(".lake.tidbcloud.com")
                    || self.host.ends_with(".lake.tidbcloud.cn")
            ...
            PresignMode::Detect => match self.get_presigned_upload_url("@~/.lakesql/check").await {
                Ok(_) => PresignMode::On,
                Err(e) => {
    ...
    ///   - (optional) 503
    ///
    /// refresh lake token or reload jwt token if needed.
...
mod test {
    ...
    async fn parse_dsn_with_private_key_uses_keypair_auth() -> Result<()> {
        ...
        let dsn = format!(
            "lake://username:password@app.tidbcloud.com/test?private_key_file={}&sslmode=disable",
    ...
    async fn parse_dsn_with_private_key_requires_user() -> Result<()> {
        ...
        let dsn = format!(
            "lake://app.tidbcloud.com/test?private_key_file={}&sslmode=disable",
            url::form_urlencoded::byte_serialize(key_file.path().to_string_lossy().as_bytes())
                .collect::<String>()
        );
    ...
    async fn parse_encoded_password() -> Result<()> {
        let dsn = "lake://username:3a%40SC(nYE1k%3D%7B%7BR@localhost";
        let client = APIClient::from_dsn(dsn).await?;
    ...
    async fn parse_special_chars_password() -> Result<()> {
        let dsn = "lake://username:3a@SC(nYE1k={{R@localhost:8000";
        let client = APIClient::from_dsn(dsn).await?;
        assert_eq!(client.host(), "localhost");
