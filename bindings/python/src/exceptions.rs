...
// Base exceptions
create_exception!(lake_driver, Warning, PyException);
create_exception!(lake_driver, Error, PyException);

// Interface errors
create_exception!(lake_driver, InterfaceError, Error);

// Database errors
create_exception!(lake_driver, DatabaseError, Error);

// Specific database error types
create_exception!(lake_driver, DataError, DatabaseError);
create_exception!(lake_driver, OperationalError, DatabaseError);
create_exception!(lake_driver, IntegrityError, DatabaseError);
create_exception!(lake_driver, InternalError, DatabaseError);
create_exception!(lake_driver, ProgrammingError, DatabaseError);
create_exception!(lake_driver, NotSupportedError, DatabaseError);

/// Map error code to appropriate PEP-249 exception based on actual Lake error code definitions
///
/// This mapping is based on real error code definitions from Lake source code
...
/// Map Lake driver errors to PEP-249 compliant exceptions based on the actual enum variants
...
/// Map lake_client::Error to PEP-249 exceptions
fn map_api_error_to_exception(api_error: &lake_client::Error, error_msg: String) -> PyErr {
    match api_error {
        // BadArgument errors are programming errors
        lake_client::Error::BadArgument(_) => ProgrammingError::new_err(error_msg),

        // IO errors are operational issues
        lake_client::Error::IO(_) => OperationalError::new_err(error_msg),

        // Request errors are operational issues (network, connectivity)
        lake_client::Error::Request(_) => OperationalError::new_err(error_msg),

        // Decode errors are typically data-related
        lake_client::Error::Decode(_) => DataError::new_err(error_msg),

        // Query execution failures - categorize by error code
        lake_client::Error::QueryFailed(error_code) => {
            map_error_code_to_exception(error_code.code, error_msg)
        }

        // Logic errors with status codes
        lake_client::Error::Logic(status, error_code) => {
            match status {
        ...
        lake_client::Error::Response { status, .. } => match status {
        ...
        lake_client::Error::QueryNotFound(_) => OperationalError::new_err(error_msg),

        // Authentication failures
        lake_client::Error::AuthFailure(_) => OperationalError::new_err(error_msg),

        // Wrapped errors - unwrap and recurse
        lake_client::Error::WithContext { inner, .. } => {
