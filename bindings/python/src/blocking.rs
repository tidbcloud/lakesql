...
use lake_driver::{LoadMethod, SchemaRef};
...
pub struct BlockingLakeClient(lake_driver::Client);

#[pymethods]
impl BlockingLakeClient {
    #[new]
    #[pyo3(signature = (dsn))]
    pub fn new(dsn: String) -> PyResult<Self> {
        let name = format!("tidbcloudlake-driver-python/{}", VERSION.as_str());
        let client = lake_driver::Client::new(dsn).with_name(name);
        Ok(Self(client))
    }

    pub fn get_conn(&self, py: Python) -> PyResult<BlockingLakeConnection> {
        let this = self.0.clone();
        let conn = wait_for_future(py, async move {
            this.get_conn().await.map_err(DriverError::new)
        })?;
        Ok(BlockingLakeConnection(Arc::new(conn)))
    }

    pub fn cursor(&self, py: Python) -> PyResult<BlockingLakeCursor> {
        let this = self.0.clone();
        let conn = wait_for_future(py, async move {
            this.get_conn().await.map_err(DriverError::new)
        })?;
        Ok(BlockingLakeCursor::new(conn))
    }
}

#[pyclass(module = "lake_driver")]
pub struct BlockingLakeConnection(Arc<lake_driver::Connection>);

#[pymethods]
impl BlockingLakeConnection {
...
/// BlockingLakeCursor is an object that follows PEP 249
/// https://peps.python.org/pep-0249/#cursor-objects
#[pyclass(module = "lake_driver")]
pub struct BlockingLakeCursor {
    conn: Arc<lake_driver::Connection>,
    rows: Option<Arc<Mutex<lake_driver::RowIterator>>>,
...
impl BlockingLakeCursor {
    fn new(conn: lake_driver::Connection) -> Self {
        Self {
            conn: Arc::new(conn),
            rows: None,
            buffer: Vec::new(),
...
impl BlockingLakeCursor {
    ...
    pub fn execute<'p>(
        ...
        let (first, rows) = wait_for_future(py, async move {
            ...
            Ok::<_, lake_driver::Error>((first, rows))
        })
        .map_err(DriverError::new)?;
