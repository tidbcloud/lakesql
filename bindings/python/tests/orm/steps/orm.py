...
"""
Step definitions for Lake ORM functionality tests
"""
...
from tidbcloudlake_driver.orm import (
    lake_model,
    rename,
    skip_serializing,
    skip_deserializing,
...
@given("I have lake-driver with ORM support")
...
def step_define_renamed_model(context: Context):
    ...
    context.model_instance = UserModel(
        id=1, username="alice", email="alice@example.com", birth_date=date(2000, 1, 1)
    )
...
def step_define_skip_serializing_model(context: Context):
    ...
    context.model_instance = SkipModel(
        id=1, name="test", created_at=datetime.now(), value="should_skip"
    )
...
def step_define_complex_model(context: Context):
    ...
    context.model_instance = ComplexModel(
        id=1, name="John Doe", email="john@example.com", updated=datetime.now()
    )
...
def step_verify_orm_helper_operations(context: Context):
    ...
    assert context.mock_connection.executed_params[0] == [
        1,
        "alice",
        "alice@example.com",
        date(1990, 1, 15),
