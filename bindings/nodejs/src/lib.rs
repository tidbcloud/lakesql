...
impl Client {
    ...
    pub fn new(dsn: String, opts: Option<ValueOptions>) -> Self {
        let name = format!("tidbcloudlake-driver-nodejs/{}", VERSION.as_str());
        let client = lake_driver::Client::new(dsn).with_name(name);
        Self {
            inner: client,
...
pub struct Connection {
    inner: Arc<lake_driver::Connection>,
    opts: ValueOptions,
}
...
impl Connection {
    pub fn new(inner: lake_driver::Connection, opts: ValueOptions) -> Self {
        Self {
            inner: Arc::new(inner),
            opts,
        }
...
#[napi]
pub struct ConnectionInfo(lake_driver::ConnectionInfo);
...
pub struct Value<'v> {
    inner: &'v lake_driver::Value,
    opts: &'v ValueOptions,
}

impl<'v> Value<'v> {
    pub fn new(inner: &'v lake_driver::Value, opts: &'v ValueOptions) -> Self {
        Self { inner, opts }
    }
}
...
impl ToNapiValue for Value<'_> {
    unsafe fn to_napi_value(env: sys::napi_env, val: Self) -> Result<sys::napi_value> {
        ...
        match val.inner {
            lake_driver::Value::Null => Null::to_napi_value(env, Null),
            lake_driver::Value::EmptyArray => {
                let arr = ctx.create_array(0)?;
                Array::to_napi_value(env, arr)
            }
            lake_driver::Value::EmptyMap => {
            ...
            lake_driver::Value::Timestamp(dt) => {
            ...
            lake_driver::Value::TimestampTz(dt) => {
                let mut js_date = std::ptr::null_mut();
                let millis = dt.timestamp().as_millisecond() as f64;
                check_status!(
                    unsafe { sys::napi_create_date(env, millis, &mut js_date) },
            ...
            lake_driver::Value::Bitmap(s) => String::to_napi_value(env, s.to_string()),
            lake_driver::Value::Variant(s) => {
            ...
            lake_driver::Value::Geometry(s) => String::to_napi_value(env, s.to_string()),
            lake_driver::Value::Interval(s) => String::to_napi_value(env, s.to_string()),
            lake_driver::Value::Geography(s) => String::to_napi_value(env, s.to_string()),
            lake_driver::Value::Vector(inner) => {
                let mut arr = ctx.create_array(inner.len() as u32)?;
                for (i, v) in inner.iter().enumerate() {
                    arr.set(
                        i as u32,
...
pub struct NumberValue(lake_driver::NumberValue);

impl ToNapiValue for NumberValue {
    unsafe fn to_napi_value(env: sys::napi_env, val: Self) -> Result<sys::napi_value> {
        match val.0 {
            lake_driver::NumberValue::Int8(i) => i8::to_napi_value(env, i),
            lake_driver::NumberValue::Int16(i) => i16::to_napi_value(env, i),
            lake_driver::NumberValue::Int32(i) => i32::to_napi_value(env, i),
            lake_driver::NumberValue::Int64(i) => i64::to_napi_value(env, i),
            lake_driver::NumberValue::UInt8(i) => u8::to_napi_value(env, i),
            lake_driver::NumberValue::UInt16(i) => u16::to_napi_value(env, i),
            lake_driver::NumberValue::UInt32(i) => u32::to_napi_value(env, i),
            lake_driver::NumberValue::UInt64(i) => u64::to_napi_value(env, i),
            lake_driver::NumberValue::Float32(i) => f32::to_napi_value(env, i),
            lake_driver::NumberValue::Float64(i) => f64::to_napi_value(env, i),
            lake_driver::NumberValue::Decimal64(_, _) => {
                String::to_napi_value(env, val.0.to_string())
            }
            lake_driver::NumberValue::Decimal128(_, _) => {
                String::to_napi_value(env, val.0.to_string())
            }
            lake_driver::NumberValue::Decimal256(_, _) => {
                String::to_napi_value(env, val.0.to_string())
            }
        }
    }
...
#[napi]
pub struct Schema(lake_driver::SchemaRef);
...
pub struct RowIterator {
    inner: lake_driver::RowIterator,
    opts: ValueOptions,
    _conn: Arc<lake_driver::Connection>,
}
...
pub struct RowIteratorExt {
    inner: lake_driver::RowStatsIterator,
    opts: ValueOptions,
    _conn: Arc<lake_driver::Connection>,
}
...
impl RowIteratorExt {
    ...
    pub async unsafe fn next(&mut self) -> Option<Result<RowOrStats>> {
        match self.inner.next().await {
            ...
            Some(r0) => match r0 {
                Ok(r1) => match r1 {
                    lake_driver::RowWithStats::Row(r) => Some(Ok(RowOrStats {
                        row: Some(Row::new(r, self.opts.clone())),
                        stats: None,
                    })),
                    lake_driver::RowWithStats::Stats(ss) => Some(Ok(RowOrStats {
                        row: None,
...
#[derive(Clone)]
pub struct Row {
    inner: lake_driver::Row,
    opts: ValueOptions,
}

impl Row {
    pub fn new(inner: lake_driver::Row, opts: ValueOptions) -> Self {
        Self { inner, opts }
    }
}
...
#[napi]
#[derive(Clone)]
pub struct ServerStats(lake_driver::ServerStats);
...
fn format_napi_error(err: lake_driver::Error) -> Error {
