...
workdir="/tmp/lakesql_put_get_paths"
src="${workdir}/src"
dst="${workdir}/dst"
...
cleanup() {
	echo "DROP STAGE IF EXISTS ${stage}" | ${LAKESQL} >/dev/null 2>&1 || true
	rm -rf "${workdir}"
}
trap cleanup EXIT

echo "DROP STAGE IF EXISTS ${stage}" | ${LAKESQL} >/dev/null
echo "CREATE STAGE ${stage}" | ${LAKESQL} >/dev/null

run_query() {
	local query="$1"
	set +e
	echo "${query}" | LAKESQL_ERROR_NO_VERSION=1 RUST_BACKTRACE=0 ${LAKESQL} --output=tsv 2>&1 |
