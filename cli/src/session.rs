...
use lake_driver::{Client, Connection, LoadMethod, ServerStats, TryFromRow};
use log::error;
...
static VERSION_SHORT: Lazy<String> = Lazy::new(|| {
    ...
    match option_env!("LAKESQL_BUILD_INFO") {
...
impl Session {
    pub async fn try_new(dsn: String, settings: Settings, is_repl: bool) -> Result<Self> {
        let client =
            Client::new(dsn.clone()).with_name(format!("lakesql/{}", VERSION_SHORT.as_str()));
        let conn = client.get_conn().await?;
        let info = conn.info().await;
        let mut keywords: Option<Arc<sled::Db>> = None;

        if is_repl {
            println!("Welcome to LakeSQL {}.", VERSION.as_str());
            ...
            let version = match conn.version().await {
                ...
                Err(err) => {
                    match err {
                        lake_driver::Error::Api(lake_client::Error::AuthFailure(_)) => {
                            return Err(err.into());
                        }
    ...
    pub async fn check(&mut self) -> Result<()> {
        // lakesql version
        {
            println!("LakeSQL {}", VERSION.as_str());
        }
        ...
        // basic connection info
        {
            let info = self.conn.info().await;
            println!(
                "Checking Lake server via {} at {}:{} as user {}.",
                info.handler, info.host, info.port, info.user
            );
            if let Some(warehouse) = &info.warehouse {
                println!("Using TiDB Cloud Lake warehouse: {warehouse}");
            }
            if let Some(database) = &info.database {
    ...
    pub async fn stream_load_stdin(&mut self, query: &str, method: LoadMethod) -> Result<()> {
        ...
        let tmp_file = dir.join(format!("lakesql_{now}"));
        {
